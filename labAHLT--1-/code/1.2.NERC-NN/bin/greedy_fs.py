...
TRAIN_ARGS = {"epochs":10,"batch_size":16,"optimizer":"Adam","lr":0.001,"embLWsize":100,"embWsize":100,"embSsize":50,"dropout_rate":0.1,"lstm_out_size":200,"linear_out_size":200,"num_layers_lstm":1,"max_len":150,"suf_len":2,"dropout_lstm":True,"linear2":False,"seed":2345, "activation":"relu", "pretrained_emb_path":None}
...
def evaluate_feature_set(feature_set):
    name= "greedy_" + ("baseline" if not feature_set else "_".join(map(str, feature_set)))
    ...
    predict(model_dir,DEVEL_PCK,pred_file,batch_size=TRAIN_ARGS["batch_size"],max_len=TRAIN_ARGS["max_len"],suf_len=TRAIN_ARGS["suf_len"],step=feature_set,affixes=AFFIXES,total_scores=TOTAL_SCORES_DEVEL)
    evaluate("NER", DEVEL_XML, pred_file, stats_file)
    f1= read_f1_from_stats(stats_file)
    return f1, name, stats_file

def greedy_forward_selection(candidates):
    selected= []
    remaining= candidates[:]
    best_f1, best_name, best_stats= evaluate_feature_set(None)
    ...
    while improved and remaining:
        ...
        for feat in remaining:
            trial= selected + [feat]
            f1, name, stats_file = evaluate_feature_set(trial)
...
def save_selected_greedy_outputs(feature_set):
    name= "selected_greedy"
    model_dir= os.path.join(MODELS_DIR, name)
...
if __name__ == "__main__":
    ...
    best_set, best_f1, history= greedy_forward_selection(CANDIDATES)

...
class SGD:
    ...
    def __init__(self, modelfile, params=None):
        ...
        if params is None :
            ...
            loss = params['loss'] if 'loss' in params else "hinge"
            alpha = float(params['alpha']) if 'alpha' in params else 0.0001
            max_iter = int(params['max_iter']) if 'max_iter' in params else 1000
            tol = float(params['tol']) if 'tol' in params else 1e-3  
            # create classifier
            self.tagger = SGDClassifier(
                            loss=loss,
                            alpha=alpha,
                            max_iter=max_iter,
                            tol=tol,

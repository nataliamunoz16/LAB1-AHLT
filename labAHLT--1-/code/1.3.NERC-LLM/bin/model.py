...
class Inference(): 
    ...
    def get_confidence_score(self, prompt_text):
        ...
        # Compute transition scores (normalized logprobs)
        # This requires transformers >= 4.26.0
        ...
        for i, token_id in enumerate(generated_tokens):
            ...
            # We target tokens involved in XML tags
            if any(m in token_str for m in ['<', '>', '/']):
                tag_probs.append(probs[i].item())

        # If the model didn't predict any tags, it's either very confident 
        # there are none, or it's totally lost. We return 0.5 as a 'neutral' confusion.
        return sum(tag_probs) / len(tag_probs) if tag_probs else 0.5
...
class FineTuning() :
    ...
    # ------------ tokenize dataset in batches of appropriate size -----------------
    def train(self, train_dataset, val_dataset, outputdir, learning_rate=2e-5, per_device_train_batch_size=1, num_train_epochs=10) :
        ''' 
